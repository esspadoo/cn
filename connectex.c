...
unsigned char flight_packet[2000];
unsigned char myip[4] = { 139,162,197,68 };
unsigned char mymac[6] ={0x22,0x00,0xb6,0xec,0x6a,0x3c};//{0xf2,0x3c,0x94,0x90,0x4f,0x4b}; // {0xf2,0x3c,0x91,0xdb,0xc2,0x98};
unsigned char mask[4] = { 255,255,255,0 };
unsigned char gateway[4] = {139,162,197,1}; //{212,71,252,1}; //{ 88,80,187,1 };
...
if((g_argc>=5) && !(rand()%INV_LOSS_RATE) && g_argv[4][0]=='S') {printf("==========TX LOST ===============\n");return 1;}
if((g_argc>=5) && (losscounter++ == 25)  &&(g_argv[4][0]=='S')){printf("==========TX LOST ===============\n");return 1;}
/**** HOST ROUTING */
...
memcpy(flight_packet, packet, 2000);
...
/*
 3.  The lost segment starting at SND.UNA MUST be retransmitted and cwnd set to ssthresh plus 3*SMSS.  This artificially "inflates"
       the congestion window by the number of segments (three) that have left the network and which the receiver has buffered. */
			...
			case FAST_RECOV:
							/*
   4.  For each additional dupliICMP Fields:
cate ACK received (after the third), cwnd MUST be incremented by SMSS.  This artificially inflates the
	...
	if ( s >= 3 && s<MAX_FD){
			...
			while(sleep(10)){
                ...
                if(fdinfo[s].tcb->st == TCP_CLOSED ){ myerrno = ECONNREFUSED; return -1;}
                    if(fdinfo[s].tcb->st == TCP_CLOSED ){myerrno = ENETUNREACH; return -1;} 
			}	
	...
	while ( 0 <= (size = recvfrom(unique_s,eth,MAXFRAME,0, (struct sockaddr *) &sll,&len))){
		...
		if (eth->type == htons (0x0806)) {
			...
			if(ip->proto == ICMP_PROTO){
                ...
                struct icmp_packet *icmp = (struct icmp_packet*) ((char*)ip + (ip->ver_ihl&0x0F)*4); 
               if(icmp->type == 3){
                   printf("ICMP TYPE 3\n");
                   unsigned int flen = 4 * ((*((unsigned char*)&flight_packet + 14))&0x0F);
	                   printf("aAAAAAAAAAAAAAAAAAAAAAA %d\n",flen);
	                   unsigned int tmpsp;
	                   memcpy(&tmpsp, flight_packet+14+flen, sizeof(tmpsp));
	                   unsigned char *quoted_ip = ((unsigned char*)icmp) + 8;
	                   unsigned int qlen = 4 * ((*quoted_ip)&0x0F);
	                   if((memcmp(&tmpsp, quoted_ip + qlen, sizeof(tmpsp))) == 0){printf("DDDDDDDDDDDDDDDDDDDDD");}
            ...
            if (ip->proto == TCP_PROTO){
			  ...
			  if(i<MAX_FD)
						...
						if((g_argc>=5) && !(rand()%INV_LOSS_RATE) && g_argv[4][0]=='C') {printf("========== RX LOST ===============\n");break;}
						fsm(i,PKT_RCV,ip);
						;//printf("status = %d\n",fdinfo[i].tcb->st);
...
//unsigned char * httpreq = "GET / HTTP/1.1\r\nHost:www.google.com\r\nConnection:close\r\n\r\n";
char * httpreq = "GET /rfcs/rfc2246.html HTTP/1.1\r\nHost:www.faqs.org\r\nConnection:close\r\nUser-Agent:curl/7.47.0\r\nAccept:*/*\r\n\r\n";
...
//addr.sin_addr.s_addr = inet_addr("213.131.64.214");// www.midor.com.eg
...
//addr.sin_addr.s_addr = inet_addr("142.251.154.119");
addr.sin_addr.s_addr = inet_addr("199.231.164.68");
//addr.sin_addr.s_addr = inet_addr("50.87.68.238");
